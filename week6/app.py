...
# 處理註冊表單 (POST)
@app.post("/signup")
def signup(request:Request, name: str = Form(...), username: str = Form(...), password: str = Form(...), session: dict=Depends(get_session), db: Session = Depends(get_db)):
    # 檢查帳號是否已經存在 
...
# 驗證登入 (POST)
@app.post("/signin")
def signin(request: Request, username: str = Form(...), password: str = Form(...), session: dict=Depends(get_session), db: Session = Depends(get_db)):
...
# 會員頁面 (GET)
@app.get("/member")
def member_page(request: Request, session: dict = Depends(get_session), db: Session = Depends(get_db)):
...
def create_message(request: Request, content: str = Form(...), session: dict = Depends(get_session), db: Session = Depends(get_db)):
    if not session.get("SIGNED-IN"):
        return RedirectResponse(url="/", status_code=303)
...
def delete_message_content(request: Request, message_id: int = Form(...), session: dict = Depends(get_session), db: Session = Depends(get_db)):
    if not session.get("SIGNED-IN"):
        return RedirectResponse(url="/", status_code=303)
    member_id = session.get("ID")
